	...
	function onDisplayField(&$field, &$item)
	{
		...
		if ($multiple) // handle multiple records
		{
			...
			foreach ($field->value as $value) {
				$field->html	.= '<li><input name="custom['.$field->name.'][]" id="'.$field->name.'" class="inputbox'.$required.'" type="text" size="'.$size.'" value="'.$value.'"'.$required.' /><input class="fcfield-button" type="button" value="'.JText::_( 'FLEXI_REMOVE_VALUE' ).'" onclick="deleteField'.$field->id.'(this);" /><span class="fcfield-drag">'.$move2.'</span></li>';
				$n++;
			}
			$field->html .=	'</ul>';
			...
			$field->html	= '<div><input name="custom['.$field->name.'][]" id="'.$field->name.'" class="inputbox'.$required.'" type="text" size="'.$size.'" value="'.$field->value[0].'"'.$required.' /></div>';
		}
	}
