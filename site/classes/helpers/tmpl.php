...
class flexicontent_tmpl
{
	...
	static function parseTemplate($tmpldir, $tmplname, &$themes)
	{
		...
		foreach ($layout_types as $layout_type => $view)
		{
			...
			if ( file_exists($tmplxml) && empty($themes->$layout_type->$tmplname) )
			{
				...
				// Do not expand document entities from template metadata. Network access
				// remains disabled and ordinary XML parsing is sufficient here.
				$doc = @simplexml_load_file($tmplxml, null, LIBXML_NONET);
				if (!$doc)
				{
				...
				//$t->params		= new \Joomla\CMS\Form\Form('com_flexicontent.template.'.$view, array('control' => 'jform', 'load_data' => true));
				...
				if ($cssfiles) {
					...
					for ($n=0; $n<count($cssfiles); $n++) {
						...
						$less_file = Path::clean( preg_replace('/^css|css$/', 'less', (string)$cssfiles[$n]) );
	...
	static function parseTemplates($tmpldir='', $force=false, $checked_layouts=array())
	{
		...
		if ( $apply_cache )
		{
			...
			$tmplcache = Factory::getCache('com_flexicontent_tmpl');  // Get Joomla Cache of '...tmpl' Caching Group
			...
			if (!empty($modified) || !empty($new_layouts) || !empty($deleted_layouts))
			{
				...
				if ($debug && !empty($modified) )        Factory::getApplication()->enqueueMessage("Re-parsing XMLs, XML file modified: ".$modified_file_list, 'message');
				if ($debug && !empty($new_layouts) )     Factory::getApplication()->enqueueMessage("Parsing new templates: ".implode(', ', $new_layouts), 'message');
				if ($debug && !empty($deleted_layouts) ) Factory::getApplication()->enqueueMessage("Cleaned cache from deleted templates: ".implode(', ', $deleted_layouts), 'message');
	...
	static function validateLayoutData($raw_data, $params, $layout = array('type'=>'item', 'name'=>'default', 'fset'=>'attribs', 'cssprep_save' => true))
	{
		...
		$layout->path = Path::clean(JPATH_SITE.DS.'components'.DS.'com_flexicontent'.DS.'templates'.DS. $layout->name .DS. $layout->type .'.xml');
		...
		if (!$xml)
		{
			Factory::getApplication()->enqueueMessage('Error parsing layout file of "' . $layout->name . '". Layout parameters were not saved', 'warning');
			return $layout_data;
		}
		...
		if (!$isValid)
		{
			Factory::getApplication()->enqueueMessage('Skipped saving of layout parameters. <br/> Error during their validation (invalid field value or required field value missing).', 'warning');
		}
