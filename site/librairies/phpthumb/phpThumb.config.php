...
$PHPTHUMB_CONFIG['high_security_password']      = '';      // FLEXIcontent generates a random key in phpThumb.config_FLEXI.php (see below). Required if 'high_security_enabled' is true, and must be at complex (uppercase, lowercase, numbers, punctuation, etc -- punctuation is strongest, lowercase is weakest; see PasswordStrength() in phpthumb.functions.php). You can use a password generator like http://silisoftware.com/tools/password-random.php to generate a strong password
...
// FLEXICONTENT MANAGED CONFIGURATION (high security flag and its key)
// This file is (re)generated by FLEXIcontent from the component configuration. Do not edit it,
// it does not exist in the installation package and it is never overwritten by FLEXIcontent updates.
if (is_file(__DIR__ . '/phpThumb.config_FLEXI.php')) {
	include(__DIR__ . '/phpThumb.config_FLEXI.php'); // plain include: the file only assigns values and must apply on every load
}

// CURRENT WEBSITE CONFIGURATION (manual overrides, loaded last so they win over everything above)
if (is_file(__DIR__ . '/phpThumb.config_OVERRIDE.php')) {
	include_once(__DIR__ . '/phpThumb.config_OVERRIDE.php');
}
