...
class plgFlexicontent_fieldsSharedaudio extends FCField
{
	...
	function onDisplayField(&$field, &$item)
	{
		...
		foreach ($field->value as $n => $value) 
		{
			...
			$value['api_type'] = @ $value['audiotype'];
			$value['media_id']  = @ $value['audioid'];
	...
	function onBeforeSaveField( &$field, &$post, &$file, &$item )
	{
		...
		foreach ($post as $n => $v)
		{
			...
			$newpost[$new]['audiotype']   = flexicontent_html::dataFilter(@$v['api_type'], 0, 'STRING', 100);
			$newpost[$new]['audioid']     = flexicontent_html::dataFilter(@$v['media_id'], 0, 'STRING', 1000);
