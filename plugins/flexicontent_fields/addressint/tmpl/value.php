...
// defaults to img for backward compatibility
...
foreach ($this->values as $n => $value) {
	...
	if (empty($map_link)) {
		...
		if (!empty($value['addr1']) && !empty($value['city']) && (!empty($value['province']) || !empty($value['state'])) && !empty($value['zip'])) {
			$map_link .= urlencode(($value['addr1'] ? $value['addr1'] . ',' : '') . ($value['city'] ? $value['city'] . ',' : '') . ($value['state'] ? $value['state'] . ',' : ($value['province'] ? $value['province'] . ',' : '')) . ($value['zip'] ? $value['zip'] . ',' : '') . ($value['country'] ? JText::_('PLG_FC_ADDRESSINT_CC_' . $value['country']) : ''));
		} else {
	...
	if ($show_map && (!empty($value['lon']) || !empty($value['lat']))) {
		...
		if ($map_embed_type == 'img') {
			$map_url = "https://maps.google.com/maps/api/staticmap?center=" . $value['lat'] . "," . $value['lon'] . "&zoom=" . ($value['zoom'] ? $value['zoom'] : $map_zoom) . "&size=" . $map_width . "x" . $map_height . "&maptype=" . $map_type . "&markers=size:" . $marker_size . "%7Ccolor:" . $marker_color . "%7C|" . $value['lat'] . "," . $value['lon'] . "&sensor=false" . ($google_maps_static_api_key ? '&key=' . $google_maps_static_api_key : '');
			$map .= '<div class="map"><div class="image">';
			if ($link_map == 1)
				$map .= '<a href="' . $map_link . '" target="_blank">';
	...
	$field -> {$prop}[$n] = $field_prefix . ($map_position == 0 && $show_map ? $map : '') . ($directions_position == 'before' && $show_address ? $map_directions : '') . ($show_address ? $addr : '') . ($directions_position == 'after' && $show_address ? $map_directions : '') . ($map_position == 1 && $show_map ? $map : '') . $field_suffix;
