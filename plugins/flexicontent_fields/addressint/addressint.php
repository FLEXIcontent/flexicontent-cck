...
class plgFlexicontent_fieldsAddressint extends FCField {
	...
	function onBeforeSaveField(&$field, &$post, &$file, &$item) {
		...
		foreach ($post as $n => $v) {
			...
			$newpost[$new]['country'] = flexicontent_html::dataFilter($v['country'], 2, 'STRING', 0);
			...
			$newpost[$new]['url'] = flexicontent_html::dataFilter($v['url'], 4000, 'STRING', 0);
			$newpost[$new]['zoom'] = flexicontent_html::dataFilter($v['zoom'], 2, 'INTEGER', 0);

			$newpost[$new]['addr2'] = !isset($v['addr2']) ? '' : flexicontent_html::dataFilter($v['addr2'], 4000, 'STRING', 0);
			$newpost[$new]['addr3'] = !isset($v['addr3']) ? '' : flexicontent_html::dataFilter($v['addr3'], 4000, 'STRING', 0);
			$newpost[$new]['state'] = !isset($v['state']) ? '' : flexicontent_html::dataFilter($v['state'], 200, 'STRING', 0);
			$newpost[$new]['province'] = !isset($v['province']) ? '' : flexicontent_html::dataFilter($v['province'], 200, 'STRING', 0);
			$newpost[$new]['zip_suffix'] = !isset($v['province']) ? '' : flexicontent_html::dataFilter($v['province'], 10, 'STRING', 0);
