	              ...
	              foreach( $list_fields as $list_fields_idx => $list_field ) {
								...
								//create field name
									...
									if($list_field->field_type === 'text') {
										$fields_display .= '<div class="field field_text"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="text" name="'.$field_name.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
										}
									if($list_field->field_type === 'email') {
										$fields_display .= '<div class="field field_email"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="email" name="'.$field_name.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.' validate-email"></div>';
										}
									if($list_field->field_type === 'date') {
											$fields_display .= '<div class="field field_date"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="date" name="'.$field_name.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
											}
									if($list_field->field_type === 'datetime-local') {
											$fields_display .= '<div class="field field_datetime"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="datetime-local" name="'.$field_name.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
											}
									if($list_field->field_type === 'textarea') {
										$fields_display .= '<div class="field field_textarea"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><textarea rows="4" cols="50" name="'.$field_name.'" '.$placeholder.' aria-label="'.$field_label.'" id="'.$field_id.'" class="'.$required.'"></textarea></div>';
										}
									if($list_field->field_type === 'radio') {
										$values_field = explode(";;",$list_field->field_value);
										$fields_display .= '<div class="field field_radio"><label for="'.$field_id.'" class="'.$class.'-radio">'.$list_field->field_label.'</label>';
									...
									if($list_field->field_type === 'checkbox') {
										$values_field = explode(";;",$list_field->field_value);
										$fields_display .= '<div class="field field_checkbox"><label for="'.$field_id.'" class="'.$class.'-checkbox">'.$list_field->field_label.'</label>';
											foreach( $values_field as $value_field ) {
												$value = JText::_($value_field);
									...
									if($list_field->field_type === 'select') {
										...
										$fields_display .= '<div class="field field_select"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><select id="'.$field_name.'" name="'.$field_name.'" aria-label="'.$field_label.'"><option value="">'.$select_label.'</option>';//TODO add required system
											foreach( $values_field as $value_field ) {
												$value = JText::_($value_field);
									...
									if($list_field->field_type === 'file') {
										$fields_display .= '<h3>NOT COMPLETE FUNCTION</h3><div class="field field_file"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="file" name="'.$field_name.'" accept="'.$value.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'" multiple></div>';//TODO add upload system
									}
									if($list_field->field_type === 'phone') {
										$fields_display .= '<div class="field field_phone"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="phone" name="'.$field_name.'" pattern="'.$value.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
									}
									...
									if($list_field->field_type === 'freehtml') {
										$fields_display .= '<div class="field field_html"><p>'.$field_label.'</p><p>'.JText::_($list_field->field_value).'</p></div>';
									}
									if($list_field->field_type === 'url') {
										$fields_display .= '<div class="field field_url"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="url" name="'.$field_name.'" pattern="'.$list_field->field_value.'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
									}
									if($list_field->field_type === 'range') {
										$values_field = explode(";;",$list_field->field_value);
										$fields_display .= '<div class="field field_range"><label for="'.$field_id.'" class="'.$class.'">'.$field_label.'</label><input type="range" name="'.$field_name.'" min="'.$values_field[0].'" max="'.$values_field[1].'" id="'.$field_id.'" '.$placeholder.' aria-label="'.$field_label.'" class="'.$required.'"></div>';
									}
							}
						}
