...
class plgFlexicontent_fieldsExtendedWeblink extends JPlugin
{
	...
	function onDisplayField(&$field, &$item)
	{
		...
		if ($multiple) // handle multiple records
		{
			...
			foreach ($field->value as $value) {
				...
				if($linktext) $linktext = '<tr><td class="key">'.JText::_( 'FLEXI_FIELD_URLLINK_TEXT' ).':</td><td><input class="urllinktext" name="'.$fieldname.'[linktext]" type="text" size="'.$size.'" value="'.($value['linktext'] ? $value['linktext'] : $field->parameters->get( 'linktext_default' )).'" /></td></tr>';
				
				$class = $field->parameters->get( 'use_class', 0 ) ? true : '';
				if($class) $class = '<tr><td class="key">'.JText::_( 'FLEXI_FIELD_URLCLASS' ).':</td><td><input class="urlclass" name="'.$fieldname.'[class]" type="text" size="'.$size.'" value="'.($value['class'] ? $value['class'] : $field->parameters->get( 'class_default', null )).'" /></td></tr>';
				
				$id = $field->parameters->get( 'use_id', 0 ) ? true : '';
				if ($id) $id = '<tr><td class="key">'.JText::_( 'FLEXI_FIELD_URLID' ).':</td><td><input class="urlid" name="'.$fieldname.'[id]" type="text" size="'.$size.'" value="'.($value['id'] ? $value['id'] : $field->parameters->get( 'id_default', null )).'" /></td></tr>';
				...
				<li>
					<table class="admintable"><tbody>
						...
						<td><input class="urllink'.$required.'" name="'.$fieldname.'[link]" type="text" size="'.$size.'" value="'.($value['link'] ? $value['link'] : $default_value).'" /></td>
						</tr>
						<tr>
						<td class="key">'.JText::_( 'FLEXI_FIELD_URLTITLE' ).':</td>
						<td><input class="urltitle'.$required.'" name="'.$fieldname.'[title]" type="text" size="'.$size.'" value="'.($value['title'] ? $value['title'] : $default_value).'" /></td>
						</tr>
						'.$linktext.'
						'.$class.'
						'.$id.'
					...
					<input class="urlhits" name="'.$fieldname.'[hits]" type="hidden" value="'.($value['hits'] ? $value['hits'] : 0).'" />
	...
	function onDisplayFieldValue(&$field, $item, $values=null, $prop='display')
	{
		...
		foreach ($values as $value)
		{
			...
			$field->{$prop}[]	= '<a href="' . JRoute::_( 'index.php?option=com_flexicontent&fid='. $field->id .'&cid='.$field->item_id.'&ord='.($n+1).'&task=weblink' ) . '"'.$id.$class.$target.' title="' . $value['title'] . '">'.( $text ).'</a>';
			$n++;
		}
