<?php
/**
 * @version 0.3 : relateditems_backlinks.php 2010-07-10 
 * @package Joomla
...
class plgFlexicontent_fieldsRelateditems_backlinks extends JPlugin
{
	...
	function onDisplayFieldValue(&$field, $item, $values=null, $prop='display')
	{
		...
		if ( !$reverse_field ) {
			$field->{$prop}  = 'Field [id:'.$filter->id.'] : ".JText::_('FLEXI_FIELD_NO_FIELD_SELECTED');
			return;
		}
		...
		$db =& JFactory::getDBO();
		$query = 'SELECT DISTINCT i.id as value, i.title as text, i.catid as catid,'
		...
		if (!$results) {
			...
			foreach($results as $result)
			{
				$displayway ? $display_text = $result->title : $display_text = $result->id;
				if($addlink){
					$link="<a href=\"".JRoute::_(FlexicontentHelperRoute::getItemRoute($itemslug,$catslug))."\" >";
					$display_text = $link .$display_text. '</a>';
				}
				$display_text = $pretext. $display_text .$posttext;
				$display[]= $display_text;
			}
	...
	function onBeforeSaveField( &$field, &$post, &$file, &$item )
	{
		...
		if ( $reverse_field || !$field->issearch ) {
			return;
		}
