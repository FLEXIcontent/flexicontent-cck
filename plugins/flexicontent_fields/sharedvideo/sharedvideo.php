...
class plgFlexicontent_fieldsSharedvideo extends FCField
{
	...
	function onDisplayField(&$field, &$item)
	{
		...
		else foreach ($field->value as $n => $value) 
		{
			...
			$value['datatype'] = @ $value['videotype'];
			$value['dataid']   = @ $value['videoid'];
		...
		function embedlyCallback_'.$field->name.'(data, element_id)
		{
			...
			if (typeof data === "object" && data.type != "error") {
				...
				{
					jQuery("#fcfield_fetching_msg_" + element_id).html("<div class=\"alert alert-warning\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">?</button>'. JText::_('PLG_FLEXICONTENT_FIELDS_SHARED'.$_MEDIA_.'_URL_NOT_VIDEO').'</div>");
					jQuery("#" + element_id + "_title, " + "#" + element_id + "_author, " + "#" + element_id + "_description, " + "#" + element_id + "_datatype, " + "#" + element_id + "_dataid, " + "#" + element_id + "_preview, " + "#" + element_id + "_width, " + "#" + element_id + "_height").parents("tr").hide("fast");
				}
			}
			else {
	...
	function onBeforeSaveField( &$field, &$post, &$file, &$item )
	{
		...
		foreach ($post as $n => $v)
		{
			...
			$newpost[$new]['videotype']   = flexicontent_html::dataFilter(@$v['datatype'], 0, 'STRING', 0);
			$newpost[$new]['videoid']     = flexicontent_html::dataFilter(@$v['dataid'], 0, 'STRING', 0);
