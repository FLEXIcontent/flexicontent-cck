...
<div class="tocNav">
	...
	if ($this->limitstart > 0 || $display_method==1) echo '
		<a href="'.($display_method==2 ? $this->prev_link : 'javascript:;').'" class="tocPrev" rel="prev" onclick="'.($display_method==1 ? 'flexibreak.previous();' : '').'">'. JText::_( 'FLEXIBREAK_PREVIOUS_PAGE' ) .'</a>';
	...
	if ($this->limitstart < $this->textscount - 1 || $display_method==1) echo '
		<a href="'.($display_method==2 ? $this->next_link : 'javascript:;').'" class="tocNext" rel="next" onclick="'.($display_method==1 ? 'flexibreak.next();' : '').'">'. JText::_( 'FLEXIBREAK_NEXT_PAGE' ) .'</a>';
	else if ($display_method==2) echo '
