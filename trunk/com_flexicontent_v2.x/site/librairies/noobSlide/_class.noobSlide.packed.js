...
this.buttons={previous:[],next:[],play:[],playback:[],stop:[]};if(params.addButtons){for(var action in params.addButtons){this.addActionButtons(action,typeOf(params.addButtons[action])=='array'?params.addButtons[action]:[params.addButtons[action]]);}}
if(this.fade)
{this.orderItems()
this.fading((params.startItem||0),true,true);}
else
{this.fx=new Fx.Tween(this.box,Object.append((params.fxOptions||{duration:500,wait:false}),{property:this.modes[this.mode][0]}));this.walk((params.startItem||0),true,true);}},orderItems:function(){for(i=0;i<this.items.length;i++)
