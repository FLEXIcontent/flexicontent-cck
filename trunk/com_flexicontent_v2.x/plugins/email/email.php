<?php
/**
 * @version 1.0 $Id: email.php 1222 2012-03-27 20:27:49Z ggppdk $
 * @package Joomla
		...
		if ($multiple) // handle multiple records
		{
			...
			foreach ($field->value as $value) {
				...
				<li>
					...
					<input class="emailaddr'.$required.'" name="custom['.$field->name.']['.$n.'][addr]" type="text" size="'.$size.'" value="'.$value['addr'].'" />
					<span class="legende">'.JText::_( 'FLEXI_FIELD_EMAILTITLE' ).':</span>
					<input class="emailtext'.$required.'" name="custom['.$field->name.']['.$n.'][text]" type="text" size="'.$size.'" value="'.@$value['text'].'" />
			...
			$field->html	= '<div>'
				...
				.': <input name="custom['.$field->name.'][0][addr]" class="emailaddr'.$required.'" type="text" size="'.$size.'" value="'.$field->value[0]['addr'].'" /> '
				.JText::_( 'FLEXI_FIELD_EMAILTITLE' )
				.': <input name="custom['.$field->name.'][0][text]" class="emailtext'.$required.'" type="text" size="'.$size.'" value="'.@$field->value[0]['text'].'" />'
				.'</div>';
		}
	}
