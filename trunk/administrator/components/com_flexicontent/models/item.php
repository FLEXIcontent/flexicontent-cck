...
class FlexicontentModelItem extends JModel
{
	...
	function _loadItem()
	{
		...
		if (empty($this->_item)) {
			...
			if($version==0) {
				/*$query = "SELECT version_id FROM #__flexicontent_versions WHERE item_id='".$this->_id."' ORDER BY version_id DESC;";
				$this->_db->setQuery($query);
			...
			//$query = "SELECT i.* FROM #__content AS i WHERE id='".$this->_id."';";
			//$this->_db->setQuery($query);
			...
			/*$obj->type_id = $type->type_id;
			$obj->language = $type->language;
			$obj->sub_items = $type->sub_items;
			$obj->sub_categories = $type->sub_categories;
			$obj->related_items = $type->related_items;
			$obj->search_index = $type->search_index;*/
			if($type) {
	...
	function store($data)
	{
		...
		if( ($isnew = !$item->id) || ((int)$post['state']==1) ) {
			$item->modified 		= $nullDate;
			$item->modified_by 	= 0;
				...
				foreach($fields as $field) {
					...
					if (is_array($post[$field->name])) {
						...
						foreach ($postvalues as $postvalue) {
							...
							if( ((int)$post['state']==1) && !isset($jcorefields[$field->field_type]) && !in_array($field->name, $jcorefields)) {
								unset($obj->version);
	...
	{
		if(!isset($this->_item->categories)||!is_array($this->_item->categories)) $this->_item->categories = array();
