 ...
 * @since       1.5
 */
class JElementFclayout extends JElement
{
	/**
	 * The form field type.
	 *
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		if (!$hideNone)    $options[] = JHTML::_('select.option', '-1', '- '.JText::_('Do not use').' -');
		if (!$hideDefault) $options[] = JHTML::_('select.option', '', '- '.JText::_('Use default').' -');
