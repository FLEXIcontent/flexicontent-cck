 ...
 */
$show_jui = ComponentHelper::getParams('com_content')->get('show_urls_images' . ($isSite ? '_frontend' : '_backend'), 0);
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_title-lbl-outer">
			<label id="jform_title-lbl" for="jform_title" data-for="jform_title" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('title')->getAttribute('label'));
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_alias-lbl-outer">
			<label id="jform_alias-lbl" for="jform_alias" data-for="jform_alias" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('alias')->getAttribute('label'));
				/* $field->label is set per type */ ?>
			</label>
		</div>
		<?php $label_html = ob_get_clean(); echo $label_html; ob_start(); ?>
...
if ((!$this->menuCats || $this->menuCats->cancatid) && $usemaincat) : ob_start();  // category ?>
	<div class="control-group">
		...
		$field_description = $field && $field->description ? $field->description : Text::_($this->form->getField('catid')->description);
		$label_attrs = 'class="' . $tip_class . $lbl_class . $lbl_extra_class . '" title="'.flexicontent_html::getToolTip(null, $field_description, 0, 1).'"';
		ob_start();
		?>
		<div class="control-label" id="jform_catid-lbl-outer">
			<label id="jform_catid-lbl" for="jform_catid" data-for="jform_catid" <?php echo $label_attrs; ?> >
				<?php
				$label_maincat = Text::_(!$secondary_displayed ? $this->form->getLabel('catid') : 'FLEXI_MAIN_CATEGORY');
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_language-lbl-outer">
			<label id="jform_language-lbl" for="jform_language" data-for="jform_language" class="<?php echo $lbl_class; ?> pull-left label-fcinner label-toplevel" >
				<?php echo $field ? $field->label : Text::_($this->form->getField('language')->getAttribute('label'));
				/* $field->label is set per type */ ?>
				<i class="icon-flag"></i>
			</label>
		</div>
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_tag-lbl-outer">
			<label id="jform_tag-lbl" data-for="input-tags" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('tag')->getAttribute('label'));
				/* $field->label is set per type */ ?>
		...
		<div class="controls container_fcfield container_fcfield_name_tags">
			...
			<?php if ($tags_editable) : ?>
				<div id="tags">
					<input type="text" id="input-tags" name="tagname" class="<?php echo $tip_class; ?>"
						placeholder="<?php echo Text::_($this->perms['cancreatetags'] ? 'FLEXI_TAG_SEARCH_EXISTING_CREATE_NEW' : 'FLEXI_TAG_SEARCH_EXISTING'); ?>"
			...
			<?php endif; ?>
				...
				<div class="fc_tagbox" id="fc_tagbox">
						...
						foreach($this->usedtagsdata as $tag)
						{
							if ($tags_editable)
							{
								...
								<li class="tagitem">
									...
									<a href="javascript:;" class="deletetag" onclick="javascript:deleteTag(this);" title="'.Text::_('FLEXI_DELETE_TAG').'"></a>
								</li>';
							}
							else
							{
					...
					{
						echo '<span class="tagicon '.$tip_class.'" title="'.Text::_('FLEXI_COMMON_TAGS').'"></span>';
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_type_id-lbl-outer">
			<label id="jform_type_id-lbl" for="jform_type_id" data-for="jform_type_id" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('type_id')->getAttribute('label'));
				/* $field->label is set per type */ ?>
		...
		<div class="controls container_fcfield container_fcfield_id_8 container_fcfield_name_type" id="container_fcfield_8">
			...
			<?php echo sprintf( $alert_box, 'id="fc-change-warning" style="display:none; float:left;"', 'warning', '', '<h4>'.Text::_( 'FLEXI_WARNING' ).'</h4> '.Text::_( 'FLEXI_TAKE_CARE_CHANGING_FIELD_TYPE' ) ); ?>
		</div>
...
if (!$is_autopublished) :  // state and vstate (= approval of new document version) ?>
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_state-lbl-outer">
			<label id="jform_state-lbl" for="jform_state" data-for="jform_state" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('state')->getAttribute('label'));
				/* $field->label is set per type */ ?>
		...
		<div class="control-group">
			...
			<div class="control-label" id="jform_vstate-lbl-outer">
				<label id="jform_vstate-lbl" data-for="jform_vstate" <?php echo $label_attrs; ?> >
					<?php echo Text::_( 'FLEXI_PUBLIC_DOCUMENT_CHANGES' ); ?>
				</label>
			</div>
			<?php $label_html = ob_get_clean(); echo $label_html; ob_start(); ?>
	...
	?>
		...
		<div class="control-group">
			...
			<div class="controls container_fcfield container_fcfield_name_vstate">
			<?php
			echo '<div class="alert alert-info">' . Text::_(($isnew || $use_versioning ? 'FLEXI_NEEDS_APPROVAL' : 'FLEXI_WITHOUT_APPROVAL') . ($isSite ? '' : '_BE')) . '</div>';
...
if ($useaccess) : ob_start();  // access ?>
	<div class="control-group">
		...
		$field_description = $field && $field->description ? $field->description : Text::_($this->form->getField('access')->description);
		$label_attrs = $field
			? 'class="' . $tip_class . $lbl_class . $lbl_extra_class . '" title="'.flexicontent_html::getToolTip(null, $field_description, 0, 1).'"'
			: 'class="' . $lbl_class . $lbl_extra_class . '"';
		ob_start();
		...
		<div class="control-label" id="jform_access-lbl-outer">
			<label id="jform_access-lbl" for="jform_access" data-for="jform_access" <?php echo $label_attrs; ?> >
				<?php echo $field ? $field->label : Text::_($this->form->getField('access')->getAttribute('label'));
				/* $field->label is set per type */ ?>
				<i class="icon-lock"></i>
			</label>
		</div>
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_attribs_comments-title-outer">
			<label id="jform_attribs_comments-title" <?php echo $label_attrs; ?> >
				<?php echo Text::_( 'FLEXI_ALLOW_COMMENTS' );?>
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_notify-lbl-outer">
			<label id="jform_notify-lbl" <?php echo $label_attrs; ?> >
				<?php echo Text::_( $isSite ? 'FLEXI_NOTIFY_FAVOURING_USERS' : 'FLEXI_NOTIFY_SUBSCRIBERS' ); ?>
				<i class="icon-mail"></i>
			</label>
		</div>
		<?php $label_html = ob_get_clean(); echo $label_html; ob_start(); ?>
	...
	<div class="control-group">
		...
		<div class="control-label" id="jform_notify_owner-lbl-outer">
			<label id="jform_notify_owner-lbl" <?php echo $label_attrs; ?> >
				<?php echo Text::_('FLEXI_NOTIFY_OWNER'); ?>
				<i class="icon-mail"></i>
			</label>
		</div>
		<?php $label_html = ob_get_clean(); echo $label_html; ob_start(); ?>
		...
		<div class="controls container_fcfield container_fcfield_name_notify">
			...
			<?php
				$tipOwnerCanEdit      = Text::_('FLEXI_OWNER_CAN_EDIT_THIS_ITEM') . ': &nbsp; - ' . mb_strtoupper(Text::_($this->ownerCanEdit ? 'JYES' : 'JNO')) . ' -';
				$tipOwnerCanEditState = Text::_('FLEXI_OWNER_CAN_PUBLISH_CHANGES_OF_THIS_ITEM') . ': &nbsp; - ' . mb_strtoupper(Text::_($this->ownerCanEdit ? 'JYES' : 'JNO')) . ' -';
			?>
...
if ( $secondary_displayed || !empty($this->lists['featured_cid']) ) : ob_start();  // categories ?>
	...
	<fieldset class="basicfields_set" id="fcform_categories_container">
		<legend>
			<?php $fset_lbl = Text::_('FLEXI_CATEGORIES') .' / '. Text::_('FLEXI_FEATURED');?>
			<span class="fc_legend_header_text"><?php echo Text::_( $fset_lbl ); ?></span>
		</legend>
			...
			<div class="control-group">
				...
				<div class="control-label" id="jform_featured_cid-lbl-outer">
					<label id="jform_featured_cid-lbl" for="jform_featured_cid" data-for="jform_featured_cid" class="<?php echo $lbl_class; ?>  pull-left label-fcinner label-toplevel">
						<?php echo Text::_( 'FLEXI_FEATURED_CATEGORIES' ); ?>
					</label>
				</div>
				<?php $label_html = ob_get_clean(); echo $label_html; ob_start(); ?>
	...
	<fieldset class="basicfields_set" id="fcform_language_container">
		<legend>
			<span class="fc_legend_header_text">
				<?php echo Text::_( 'FLEXI_LANGUAGE' ) . ' '. Text::_( 'FLEXI_ASSOCIATIONS' ) ; ?>
			</span>
		</legend>
		...
		<?php if (flexicontent_db::useAssociations() && $modifyLangAssocs): ?>
			...
			<?php else: ?>
				<?php echo Text::_( 'FLEXI_ASSOC_NOT_POSSIBLE' ); ?>
			<?php endif; ?>
		<?php endif; ?>
...
if ($typeid && $this->perms['canright'] && $usepublicationdetails === 2) : ob_start(); ?>
		...
		<tr>
			<td colspan="2">
				<h3><?php echo Text::_( 'FLEXI_VERSION_INFO' ); ?></h3>
			</td>
		</tr>
		<?php
		if ( $this->row->id ) {
		...
		<tr>
			<td class="key">
				<label class="fc-prop-lbl"><?php echo Text::_( 'FLEXI_ITEM_ID' ); ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key">
				...
				?>
				<label <?php echo $label_attrs; ?>><?php echo $field ? $field->label : Text::_( 'FLEXI_STATE' ); ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key">
				<?php
					...
					$field_description = $field && $field->description ? $field->description : Text::_($this->form->getField('hits')->description);
					$label_attrs = $field
						? 'class="' . $tip_class . ' fc-prop-lbl" title="'.flexicontent_html::getToolTip(null, $field_description, 0, 1).'"'
						: 'class="' . $default_label_class . '"';
				?>
				<label <?php echo $label_attrs; ?>><?php echo $field ? $field->label : Text::_( 'FLEXI_HITS' ); ?></label>
			</td>
			<td>
				<div id="hits" style="float:left;" class="badge badge-info"><?php echo $this->row->hits; ?></div> &nbsp;
				<span <?php echo $visibility; ?>>
					<input name="reset_hits" type="button" class="button btn-small btn-warning" value="<?php echo Text::_( 'FLEXI_RESET' ); ?>" onclick="reseter('<?php echo $ctrl_items; ?>resethits', '<?php echo $this->row->id; ?>', 'hits')" />
				</span>
			</td>
		</tr>
		<tr>
			<td class="key">
				<?php
					...
					$label_attrs = $field
						...
						: 'class="' . $default_label_class . '"';
				?>
				<label <?php echo $label_attrs; ?>><?php echo $field ? $field->label : Text::_( 'FLEXI_SCORE' ); ?></label>
			</td>
			<td>
				<div id="votes" style="float:left;" class="badge badge-info"><?php echo $this->ratings; ?></div> &nbsp;
				<span <?php echo $visibility2; ?>>
					<input name="reset_votes" type="button" class="button btn-small btn-warning" value="<?php echo Text::_( 'FLEXI_RESET' ); ?>" onclick="reseter('<?php echo $ctrl_items; ?>resetvotes', '<?php echo $this->row->id; ?>', 'votes')" />
				</span>
			</td>
		</tr>
		...
		<tr>
			<td class="key">
				<label class="<?php echo $default_label_class; ?>"><?php echo Text::_( 'FLEXI_REVISED' ); ?></label>
			</td>
			<td>
				<span class="badge badge-info">
					<?php echo $this->row->last_version;?> <?php echo Text::_( 'FLEXI_TIMES' ); ?>
				</span>
			</td>
		</tr>
		<tr>
			<td class="key">
				<label class="<?php echo $default_label_class; ?>"><?php echo Text::_( 'FLEXI_FRONTEND_ACTIVE_VERSION' ); ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key">
				<label class="<?php echo $default_label_class; ?>"><?php echo Text::_( 'FLEXI_FORM_LOADED_VERSION' ); ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key">
				<?php
					...
					$field_description = $field && $field->description ? $field->description : Text::_($this->form->getField('created')->description);
					$label_attrs = $field
						? 'class="' . $tip_class . ' fc-prop-lbl" title="'.flexicontent_html::getToolTip(null, $field_description, 0, 1).'"'
						: 'class="' . $default_label_class . '"';
				?>
				<label <?php echo $label_attrs; ?>><?php echo $field ? $field->label : Text::_( 'FLEXI_CREATED' ); ?></label>
			</td>
			<td>
				<?php echo $this->row->created == $this->nullDate
					? Text::_( 'FLEXI_NEW_ITEM' )
					: HTMLHelper::_('date',  $this->row->created,  Text::_( 'DATE_FORMAT_LC2' ) ); ?>
			</td>
		</tr>
		<tr>
			<td class="key">
				<?php
					...
					$field_description = $field && $field->description ? $field->description : Text::_($this->form->getField('modified')->description);
					$label_attrs = $field
						? 'class="' . $tip_class . ' fc-prop-lbl" title="'.flexicontent_html::getToolTip(null, $field_description, 0, 1).'"'
						: 'class="' . $default_label_class . '"';
				?>
				<label <?php echo $label_attrs; ?>><?php echo $field ? $field->label : Text::_( 'FLEXI_MODIFIED' ); ?></label>
			</td>
			<td>
				<?php
					if ( $this->row->modified == $this->nullDate ) {
						echo Text::_( 'FLEXI_NOT_MODIFIED' );
					} else {
						echo HTMLHelper::_('date',  $this->row->modified, Text::_( 'DATE_FORMAT_LC2' ));
					}
				?>
			</td>
		</tr>
	<?php if ($use_versioning) : ?>
			<tr>
				<td class="key">
					<label class="<?php echo $default_label_class; ?>"><?php echo Text::_( 'FLEXI_VERSION_COMMENT' ); ?></label>
				</td>
				<td></td>
			</tr><tr>
...
if ($usepublicationdetails) : // timezone_info, publication_details ?>
	...
	<?php ob_start(); ?>
		...
		$tz_offset = $tz->getOffset(new Date()) / 3600;
		$tz_info =  $tz_offset > 0 ? ' UTC +' . $tz_offset : ' UTC ' . $tz_offset;

		$tz_info .= ' ('.$user_zone.')';
		$msg = Text::sprintf( FLEXI_J16GE ? 'FLEXI_DATES_IN_USER_TIMEZONE_NOTE' : 'FLEXI_DATES_IN_SITE_TIMEZONE_NOTE', ' ', $tz_info );
...
<?php endif; ?>
	...
	<?php foreach ($displayed_fieldSets as $name => $fieldSet) :
		...
		$label = Text::_($label) === 'COM_FLEXICONTENT_'.$name.'_FIELDSET_LABEL'
...
if ($typeid && $selecttheme) : ?>
	...
	<?php if ( $selecttheme >= 1 ) : ob_start();
		...
		<div class="fc-success fc-mssg-inline" style="font-size: 12px; margin: 8px 0 !important;" id="__content_type_default_layout__">
			<?php /*echo Text::sprintf( 'FLEXI_USING_CONTENT_TYPE_LAYOUT', $this->tparams->get('ilayout') ) . "<br/><br/>";*/ ?>
			<?php echo Text::_($isSite ? 'FLEXI_USING_LAYOUT_DEFAULTS' : 'FLEXI_RECOMMEND_CONTENT_TYPE_LAYOUT'); ?>
		</div>
	<?php
	$fn = 'layout_selection';
	$captured[$fn] = ob_get_clean();
	...
	if ( $selecttheme >= 2 ) : ob_start(); ?>
		...
		<div class="fc-sliders-plain-outer <?php echo $item_layout ? 'fc_preloaded' : ''; ?>">
			...
			//echo HTMLHelper::_('sliders.start', $slider_set_id, array('useCookie'=>1));
			echo HTMLHelper::_('bootstrap.startAccordion', $slider_set_id, array(/*'active' => ''*/));
			...
			foreach ($this->tmpls as $tmpl) :
				...
				//echo HTMLHelper::_('sliders.panel', $slider_title, $slider_id);
				echo HTMLHelper::_('bootstrap.addSlide', $slider_set_id, $slider_title, $slider_id);
			...
			<?php echo HTMLHelper::_('bootstrap.endAccordion'); //echo HTMLHelper::_('sliders.end'); ?>
...
if ($typeid && $use_versioning && $this->perms['canversion'] && $versionsplacement !== 0) : ob_start()  // versions ?>
	<table class="" style="margin: 10px; width: auto;">
		<tr>
			<td>
				<h3><?php echo Text::_( 'FLEXI_VERSIONS_HISTORY' ); ?></h3>
			</td>
		</tr>
		<tr><td>
			...
			<tr>
				<td class="versions-first" colspan="4"><?php echo Text::_( 'FLEXI_NEW_ARTICLE' ); ?></td>
			</tr>
			<?php
			else :
			$date_format = (($date_format = Text::_( 'FLEXI_DATE_FORMAT_FLEXI_VERSIONS_J16GE' )) == 'FLEXI_DATE_FORMAT_FLEXI_VERSIONS_J16GE') ? "d/M H:i" : $date_format;
			...
			<tr<?php echo $class; ?>>
				...
				<td class="versions"><span style="padding: 0 5px 0 0;"><?php echo HTMLHelper::_('date', (($version->nr == 1) ? $this->row->created : $version->date), $date_format ); ?></span></td>
				...
				<td class="versions">
					...
					<a href="javascript:;" class="hasTooltip" title="<?php echo HTMLHelper::tooltipText( Text::_( 'FLEXI_COMMENT' ), ($version->comment ? $version->comment : 'No comment written'), 0, 1); ?>"><?php echo $comment_image;?></a>
					...
					<?php if (!$isSite) :?>
						...
						<?php if (!$isCurrent && $allow_versioncomparing) : ?>
							<a class="modal-versions"
								...
								title="<?php echo Text::_( 'FLEXI_COMPARE_WITH_CURRENT_VERSION' ); ?>"
							>
						...
						<?php if ($isCurrent) : ?>
							<a onclick="javascript:return clickRestore('<?php echo Uri::base(true); ?>/index.php?option=com_flexicontent&amp;view=item&amp;<?php echo $task_items;?>edit&amp;<?php echo ($isSite ? 'id=' : 'cid=') . $this->row->id;?>&amp;version=<?php echo $version->nr; ?>');" href="javascript:;"><?php echo Text::_( 'FLEXI_CURRENT' ); ?></a>
						<?php else : ?>
							<a onclick="javascript:return clickRestore('<?php echo Uri::base(true); ?>/index.php?option=com_flexicontent&amp;task=items.edit&amp;<?php echo ($isSite ? 'id=' : 'cid=') . $this->row->id;?>&amp;version=<?php echo $version->nr; ?>&amp;<?php echo \Joomla\CMS\Session\Session::getFormToken();?>=1');"
								href="javascript:;"
								title="<?php echo Text::sprintf( 'FLEXI_REVERT_TO_THIS_VERSION', $version->nr ); ?>"
							>
...
if ($permsplacement && $this->perms['canright'] ) : ob_start(); // perms ?>
	<fieldset id="flexiaccess" class="flexiaccess basicfields_set">
		<legend><?php echo Text::_( 'FLEXI_RIGHTS_MANAGEMENT' ); ?></legend>
		<div id="tabacces">
			<div id="accessrules"><?php echo $this->form->getInput('rules'); ?></div>
		</div>
		<?php if ($permsplacement === 2) : ?>
		<div id="notabacces">
			<?php echo Text::_( 'FLEXI_RIGHTS_MANAGEMENT_DESC' ); ?>
		</div>
		<?php endif; ?>
	</fieldset>
<?php
			...
			if ($field->field_type === 'image')
			{
				if ($field->parameters->get('image_source')==-1)
				{
					$replace_txt = !empty($captured['jimages'])
						? $captured['jimages']
						: sprintf( $alert_box, '', 'warning', 'fc-nobgimage', Text::_('FLEXI_ENABLE_INTRO_FULL_IMAGES_IN_TYPE_CONFIGURATION') );
			...
			if ($field->field_type === 'weblink')
			{
				if ($field->parameters->get('link_source')==-1)
				{
					$replace_txt = !empty($captured['jurls'])
						? $captured['jurls']
						: sprintf( $alert_box, '', 'warning', 'fc-nobgimage', Text::_('FLEXI_ENABLE_LINKS_IN_TYPE_CONFIGURATION') );
...
<?php else : /* NO TYPE SELECTED */ ?>
		...
		<?php if ( $typeid == 0) : // type_id is not set (user allowed to select item type) ?>
			...
			<?php echo sprintf( $alert_box, '', 'info', '', Text::_( 'FLEXI_CHOOSE_ITEM_TYPE' ) ); ?>
		<?php else : // existing item that has no custom fields, warn the user ?>
			<?php echo sprintf( $alert_box, '', 'info', '', Text::_( 'FLEXI_NO_FIELDS_TO_TYPE' ) ); ?>
		<?php endif; ?>
