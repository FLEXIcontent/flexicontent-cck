...
<?php if (!$this->folder_mode) : ?>
	<table class="adminform fileselement-list-filters" border="0">
		<tr>
			<td align="left">
				<label class="label"><?php echo JText::_( 'FLEXI_SEARCH' ); ?></label>
				<?php echo $this->lists['filter']; ?>
				<input type="text" name="search" id="search" value="<?php echo htmlspecialchars($this->lists['search'], ENT_QUOTES, 'UTF-8'); ?>" class="text_area" onChange="document.adminForm.submit();" />
...
<?php } ?>
	...
	<tbody>
		...
		foreach ($this->rows as $row) {
			...
			if ($this->folder_mode) {
				$img_assign_link = "window.parent.qmAssignFile".$this->fieldid."('".$this->targetid."', '".$filename."', '".$file_preview."');document.getElementById(\"file{$k}\").className=\"striketext\";";
			} else {
