...
/**
 * HTML View class for the mediadatas backend manager
 */
#[AllowDynamicProperties]
class FlexicontentViewMediadatas extends FlexicontentViewBaseRecords
{
		/** @var mixed $contentRows */
	public mixed $contentRows = null;
...
var $proxy_option   = null;
	...
	var $db_tbl         = 'flexicontent_mediadatas';
	var $name_singular  = 'mediadata';
	...
	public function display($tpl = null)
	{
		...
		$filter_media_type  = $model->getState('filter_media_type');

		if (strlen($filter_state)) $count_filters++;
		if (strlen($filter_media_type)) $count_filters++;
		...
		FLEXIUtilities::ManagerSideMenu('CanMediadatas');

		// Create document/toolbar titles
		$doc_title = \Joomla\CMS\Language\Text::_('FLEXI_MEDIADATAS');
		$site_title = $document->getTitle();
		\Joomla\CMS\Toolbar\ToolbarHelper::title($doc_title, 'mediadatas');
		...
		// Get content items related to mediadatas
		...
		$options[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option',  '1', \Joomla\CMS\Language\Text::_( 'Video' ) );
		$options[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option',  '0', \Joomla\CMS\Language\Text::_( 'Audio' ) );

		$fieldname = 'filter_media_type';
		$elementid = 'filter_media_type';

		$lists[$elementid] = $this->getFilterDisplay(array(
			'label' => \Joomla\CMS\Language\Text::_('FLEXI_MEDIADATA_TYPE'),
			'html' => \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist',
				$options,
				$fieldname,
				array(
	...
	function setToolbar()
	{
		...
		$hasCreate    = $perms->CanCreateMediadatas;
		$hasEdit      = $perms->CanMediadatas;
		$hasEditState = $perms->CanMediadatas;
		$hasDelete    = $perms->CanMediadatas;
