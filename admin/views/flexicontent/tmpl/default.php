...
if (version_compare(PHP_VERSION, FLEXI_PHP_NEEDED, '<')) {
	$app->enqueueMessage(JText::sprintf('FLEXI_UPGRADE_PHP_VERSION_GE', FLEXI_PHP_NEEDED), 'error');
	return false;
}
				...
				<?php else : ?>
					...
					if ($this->dopostinstall && isset($php_lims['warning']) && $config_saved) {
						...
						if ($phplimits_printed < 1) {
							$mssg = '<h4 class="alert-heading">PHP/DB Requirements</h4>';
							foreach ($php_lims as $type => $html) {
								$mssg .= implode('<br>', $html);
							}
							$mssg .= JText::sprintf(
								'<p><br>(you may have to contact your web hosting company for setting these for you)<br>
					For more information on changing these limitations, please see this article: %s',
								'<a href="https://www.flexicontent.org/documentation/tutorials-english/78-installation-upgrade/591-php-db-requirements.html>PHP/DB Requirements</a><p>'
							);
					...
					if (isset($php_lims['warning']))
						$_title .= '<span class="badge bg-error ms-2">Warning</span>';
					else if (isset($php_lims['notice']))
						$_title .= '<span class="badge bg-warning badge-warning ms-2">Notice</span>';
					else
						$_title .= '<span class="badge bg-success ms-2">OK</span>';
					?>
					...
					<div class="row">
						...
						<div id="fc-dash-boardbtns" class="col-xl-7">
							<?php
							if (!$this->dopostinstall) {
								$app->enqueueMessage(JText::_('FLEXI_DO_POSTINSTALL'), 'warning');
								...
								if (!$config_saved) {
									...
									$message .= JText::sprintf('FLEXI_CONFIGURATION_NOT_SAVED', $conf_link . JText::_("FLEXI_CONFIG") . '</a>') . '<br>';
								}
								if (!$this->existcat) $message .= JText::_('FLEXI_NO_CATEGORIES_CREATED') . '<br>';
								if (!$this->existmenu) $message .= JText::_('FLEXI_NO_MENU_CREATED');

								if (!empty($message)) $app->enqueueMessage($message, 'warning');
							}
										...
										<div class="fc-board-set-inner"><?php
																		...
																		if (!isset($sbtns['additem'])) {
																			...
																			if ($CanAddAny) {
																				...
																				FlexicontentViewFlexicontent::quickiconButton($link, '', 'fas fa-file-medical', JText::_('FLEXI_NEW_ITEM'), 1, 1, 1200, 0);
																			}
																		}
																		...
																		if ($this->perms->CanCats) {
																			...
																			if (!isset($sbtns['addcat'])) {
																				...
																				if ($canCreateAny) {
																					...
																					FlexicontentViewFlexicontent::quickiconButton($link, '', 'fas fa-folder-plus', JText::_('FLEXI_NEW_CATEGORY'));
																				}
																			}
																		}
										...
										<div class="fc-board-set-inner"><?php
																		...
																		if ($this->perms->CanFields) {
																			if (!isset($sbtns['fields'])) {
																				...
																				FlexicontentViewFlexicontent::quickiconButton($link, '', 'icon-puzzle', JText::_('FLEXI_FIELDS'));
																				$add_sep = true;
																			}
				...
				FlexicontentViewFlexicontent::quickiconButton( $com_xml['authorUrl'], 'icon-48-dashboard.png', JText::_( 'FLEXI_ABOUT' ), 1 );
						...
						<div class="col-xl-5 mt-5">
							...
							elseif (!$skip_sliders && $config_saved) :
								...
								$message = '<h4 class="alert-heading">PHP/DB Requirements</h4>';
								foreach ($php_lims as $type => $html) {
									$message .= implode('<br>', $html);
								}
								$message .= JText::sprintf(
									'<p><br>(you may have to contact your web hosting company for setting these for you)<br>
						For more information on changing these limitations, please see <a href="https://www.flexicontent.org/documentation/tutorials-english/78-installation-upgrade/591-php-db-requirements.html" target="_blank">this article.</a>'
								);
								...
								<?php if (!isset($ssliders['pending'])) :
									$badge_class = "";
									if ($this->totalrows['pending'] == 0) {
										$badge_class = " disabled";
									}
									$title = JText::_('FLEXI_PENDING_SLIDER2') . '<span class="badge bg-warning badge-warning' . $badge_class . '">' . $this->totalrows['pending'] . '</span>';
										...
										<tbody>
											...
											?>
												<tr>
													...
													<td><?php echo JHtml::_('date',  $row->created, $date_format); ?></td>
								...
								<?php if (!isset($ssliders['revised'])) :
									$badge_class = "";
									if ($this->totalrows['revised'] == 0) {
										$badge_class = " disabled";
									}
									$title = JText::_('FLEXI_REVISED_VER_SLIDER2') . '<span class="badge bg-warning badge-warning' . $badge_class . '">' . $this->totalrows['revised'] . '</span>';
										...
										<tbody>
											...
											?>
												<tr>
													...
													<td><?php echo JHtml::_('date',  $row->modified, $date_format); ?></td>
								...
								<?php if (!isset($ssliders['inprogress'])) : ?>
									<?php
									$badge_class = "";
									if ($this->totalrows['inprogress'] == 0) {
										$badge_class = " disabled";
									}
									$title = JText::_('FLEXI_IN_PROGRESS_SLIDER2') . '<span class="badge bg-info badge-info-2' . $badge_class . '">' . $this->totalrows['inprogress'] . '</span>';
								...
								<?php if (!isset($ssliders['draft'])) : ?>
									<?php
									$badge_class = "";
									if ($this->totalrows['draft'] == 0) {
										$badge_class = " disabled";
									}
									$title = JText::_('FLEXI_DRAFT_SLIDER2') . '<span class="badge bg-info badge-info-2' . $badge_class . '">' . $this->totalrows['draft'] . '</span>';
					...
					<!-- aaa -->
					<?php
					if (!$skip_sliders && $config_saved) :

						echo JHtml::_('bootstrap.startAccordion', 'fc-dash-sliders-2', array());
				...
				");
							...
							<div class="fc-board-set-inner">
								...
								<div class="row">
									...
									if (!$disable_fc_logo) {
										...
										echo JHtml::image('administrator/components/com_flexicontent/assets/images/logo.png', 'FLEXIcontent', array('id' => 'fc-dash-logo'));
										echo '</div>';
									}
									...
									<div class="col">
										<div class="alert alert-info">
											FLEXIcontent <?php echo FLEXI_VERSION . ' ' . FLEXI_RELEASE; ?><br /> GNU/GPL licence, Copyright &copy; 2009-<?php echo date('Y'); ?>
										</div>
									</div>
								</div>
								...
								<div class="row" id="fc-dash-devs">
									<div class="col">
										<p class="text-nowrap"><span class="fw-semibold <?php echo $tooltip_class; ?>" title="Core developer">Georgios Papadakis</span><br>
											<a class="<?php echo $btn_class . ' btn-sm btn-primary ' . $tooltip_class; ?>" href="http://www.flexicontent.org" title="FLEXIcontent home page" target="_blank">FLEXIcontent.org</a>
										</p>
									</div>
									<div class="col">
										<p class="text-nowrap"><span class="fw-semibold <?php echo $tooltip_class; ?>" title="Core developer">Emmanuel Danan</span><br>
											<a class="<?php echo $btn_class . ' btn-sm btn-primary ' . $tooltip_class; ?>" href="http://www.agerix.fr" title="Agerix : L'agence digitale gauloise<br>Spécialistes Joomla - conception, refonte, migration et maintenance." target="_blank">agerix.fr</a>
										</p>
									</div>
									<div class="col">
										<p class="text-nowrap"><span class="fw-semibold <?php echo $tooltip_class; ?>" title="Core Developer">Marvelic Engine</span><br>
											<a class="<?php echo $btn_class . ' btn-sm ' . $tooltip_class; ?>" href="http://www.marvelic.co.th" target="_blank" title="<?php echo flexicontent_html::getToolTip("Marvelic Engine", "Marvelic Engine is a Joomla consultancy based in Bangkok, Thailand. Support services include consulting, Joomla implementation, training, and custom extensions development.", 0, 1); ?>">
												marvelic.co.th
											</a>
										</p>
									</div>
									<div class="col">
										<p class="text-nowrap"><span class="label <?php echo $tooltip_class; ?>" title="Core Developer">Suriya Kaewmungmuang</span></p>
									</div>
									<div class="col">
										<p class="text-nowrap"><span class="label <?php echo $tooltip_class; ?>" title="Core Developer">Yannick Berges</span><br>
											<a class="<?php echo $btn_class . ' btn-sm ' . $tooltip_class; ?>" href="http://com3elles.com/" target="_blank" title="<?php echo flexicontent_html::getToolTip("Com'3Elles", "Com'3Elles, agence de communication, conseil et formations", 0, 1); ?>">
												com3elles.com
											</a>
										</p>
									</div>
									...
									<div class="col">
										...
										<p class="text-nowrap"><span class="label <?php echo $tooltip_class; ?>" title="Developer / Contributor">Ruben Reyes</span><br>
											<a class="<?php echo $btn_class . ' btn-sm ' . $tooltip_class; ?>" href="http://www.lyquix.com" target="_blank" title="<?php echo flexicontent_html::getToolTip("Lyquix", "Lyquix - Philadelphia Marketing, Advertising, Web Design and Development Agency", 0, 1); ?>">
												lyquix.com
											</a>
										</p>
					...
					<?php
						...
						if ($hide_fc_license_credits) :
							echo JHtml::_('bootstrap.addSlide', 'fc-dash-sliders', "About FLEXIcontent", 'fc-dash-sliders-aboutflexi', false);
