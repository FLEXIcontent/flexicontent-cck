...
abstract class JHtmlFctags
{
	...
	public static function edit_link_jrecord($row, $i, $ctrl, $canEdit)
	{
		...
		{
			$onclick   = "var url = jQuery(this).attr('data-href'); var the_dialog = fc_showDialog(url, 'fc_modal_popup_container', 0, 0, 0, fc_edit_jtag_modal_close, {title:'" . JText::_('FLEXI_EDIT_JTAG') . "', loadFunc: fc_edit_jtag_modal_load}); return false;";
			$edit_link = 'index.php?option=com_tags&amp;task=tag.edit&amp;id=' . $row->jtag_id;

			return '
			<a href="javascript:;" onclick="' . $onclick . '" data-href="' . $edit_link . '" ' . $common_attrs . '>
				<span class="icon-pencil"></span>
			</a>';
		}
	}
