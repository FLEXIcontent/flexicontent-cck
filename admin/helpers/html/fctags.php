...
abstract class JHtmlFctags
{
	...
	public static function published($value = 0, $i, $canChange = true)
	{
		...
		if ($states === null)
		{
			...
			$states = array(
				0 => array('unpublish', 'tags.publish', JHtml::tooltipText('JUNPUBLISHED'), JHtml::tooltipText('JPUBLISHED')),
				1 => array('publish', 'tags.unpublish', JHtml::tooltipText('JPUBLISHED'), JHtml::tooltipText('JUNPUBLISHED')),
			);
		}
		$value = (int) $value;
	...
	{
		//return JHtml::_('jgrid.checkedout', $i, $row->editor, $row->checked_out_time, 'tags.', $row->canCheckin);
		...
		<a class="btn btn-micro ntxt hasTooltip" title="'.$_tip_title.'" href="javascript:;" onclick="var ccb=document.getElementById(\'cb'.$i.'\'); ccb.checked=1; ccb.form.task.value=\'tags.checkin\'; ccb.form.submit();">
