...
echo USAGE:
echo.
echo  %0  /J JOOMLA_SITE_PATH  /R FLEXI_REPOSITORY_PATH
exit /b
...
FOR /D %%G in ("*") DO (
	...
	IF EXIST  %joomlasite_path%\language\en-GB\en-GB.%%G.ini_  (
		...
		ren  %joomlasite_path%\language\en-GB\en-GB.plg_%%G.ini  en-GB.plg_%%G.ini_
	)
	mklink %joomlasite_path%\language\en-GB\en-GB.plg_%%G.ini  %repository_path%\modules\language\%%G\en-GB.plg_%%G.ini
)
