				...
				<?php foreach ($list[$ord]['standard'] as $item) : ?>
					...
					?>
						<span class="<?php echo $classes; ?>" title="<?php echo $tip_html; ?>" >
							...
							// Signed phpThumb URLs (HTML escaped, valid inside src and srcset attributes)
							$_hurl = function ($f) use ($_hb, $_handle_raw, $_hw, $_hh) {
								return flexicontent_images::phpThumbURL(array('src' => $_hb . $_handle_raw, 'w' => $_hw, 'h' => $_hh, 'aoe' => 1, 'q' => 85, 'zc' => 1, 'f' => $f));
							};
							$_style_h = 'width:'.$_hw.'px; height:'.$_hh.'px; display:block; object-fit:cover;';
							echo '<picture>'
								. '<source type="image/webp" srcset="'.$_hurl('webp').'">'
								. '<img src="'.$_hurl('jpg').'" alt="" width="'.$_hw.'" height="'.$_hh.'" style="'.$_style_h.'" loading="lazy" decoding="async" />'
								. '</picture>';
							?>
						</span>
					<?php endforeach; ?>
